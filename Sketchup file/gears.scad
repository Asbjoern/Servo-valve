...
module gear (
	...
	assign(k  = -iang(b, p) - t/2/p/pi*180) {             //angle to where involute meets base circle on each side of tooth
		difference() {
			...
			cylinder(h=2*thickness+1, r=hole_diameter/2, center=true, $fn=96);
		}
	}
};	
...
n1 = 27; //red gear number of teeth
n2 = 36; //green gear

mm_per_tooth = 3.7; //all meshing gears need the same mm_per_tooth (and the same pressure_angle)
thickness    = 6;
hole         = 1;
...
translate([ 0,    0, 0]) rotate([0,0, $t*360/n1])                 color([1.00,0.75,0.75]) gear(mm_per_tooth,n1,thickness,9);
translate([ 0,  d12, 0]) rotate([0,0,-($t+n2/2-0*n1+1/2)*360/n2]) color([0.75,1.00,0.75]) gear(mm_per_tooth,n2,thickness,5.8);
